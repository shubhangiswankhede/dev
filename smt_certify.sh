...
main(){
    ...
    local container_job="_start_container_pipeline"
    ...
    then
         ...
         echo "Getting the latest good artifact from ${container_job}"
         get_latest_good_artefact "${container_job}" "$chefdir" 
    else
         echo "-----------------------------------------------------------"
    ...
    IP_ADDRESS=$vm_ip HOSTNAME=$vm_host\
         ...
         jsons/${JSON_TEMPLATE}.json > $chefdir/nodes/node.json
    ...
    pushd ${cucumber_dir}
        ...
        env_ip=$vm_ip env_user=$USERNAME env_pass=$PASSWORD\
            cucumber -t "@${CERTIFY_FEATURES}" $cucumber_opts
        [[ $? != 0 ]] && { 
