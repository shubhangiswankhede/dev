...
main(){
    ...
    IP_ADDRESS=$vm_ip HOSTNAME=$vm_host\
         ...
         ${chefdir}/jsons/${JSON_TEMPLATE}.json > $chefdir/nodes/node.json
    ...
    then
        ...
        "sudo su - ${USERNAME} bash -c \"rsync -lrp --delete /tmp/${chefdir} ~\""
        [[ $? != 0 ]] && exit 1

        # Executing the cookbook
        ssh -t -t -o 'StrictHostKeyChecking=no' ${USERNAME}@${vm_ip} -- \
        "sudo su - ${USERNAME} bash -c \"cd ~/${chefdir};sudo ${USERNAME} -j nodes/node.json -c solo.rb -l info -Fdoc\""
        [[ $? != 0 ]] && exit 1
        ...
        # Executing the cookbook
        ssh -t -t -o 'StrictHostKeyChecking=no' ${USERNAME}@${vm_ip} -- \
        "cd ~/${chefdir};sudo chef-solo -j nodes/node.json -c solo.rb -l info -Fdoc"
        [[ $? != 0 ]] && exit 1
    fi
