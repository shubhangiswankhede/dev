...
#add_vappids_to_runlist(){
#    local ips=($1);
#
#    lc-node-list > $node_list
#    for ((i=0; i<${#ips[*]}; i++))
#    do
#        echo "Adding vappid to runlist for ${ips[$i]}"
#        VAPP_LIST+="$vm_ip "
#
#}
...
create_vm_for_jobs(){
    ...
    get_param_value_from_settings "vapp_template_id" "$tmp_file"
    ...
    get_param_value_from_settings "user_name" "$tmp_file"
    local username=`cat $tmp_file`
    get_param_value_from_settings "email" "$tmp_file"
    local email=`cat $tmp_file`
    ...
    do
        ...
        provision_vm "$vapp_template_id" "$vapp_node_size" "$vapp_network_id" "$vapp_storage_profile"\
            "${jobs[$i]}-${BUILD_NUMBER}" "$channel_code" "$application_code" "$org_code" "$group_code" "$username" "$email" || { exit 1; }
        vm=($vmstr)
        vm_id=${vm[0]}
        vm_name=${vm[1]}
        vm_ip=${vm[2]}
...
main(){
    ...
    echo "VAPP_LIST=$VAPP_LIST" >> $TRIGGER_PROPERTIES
